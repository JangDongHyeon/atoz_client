...
const FaqUpdate = ({ history, match }) => {
    const { imagePaths, updateFaqsDone, faq } = useSelector(state => state.faq);
    ...
    const onSubmit = useCallback(() => {
        ...
        formData.append('faqId', match.params.faqId)
        return dispatch({
    ...
    const onRemoveImage = useCallback((index, p) => () => {
        ...
        if (p)
            if (p.id)
                dispatch({
                    type: FAQ_REMOVE_UPDATE_IMAGE_REQUEST,
                    data: { faqId: p.id },
                })
